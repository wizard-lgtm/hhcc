...
def handle_function_definition(
    ...
    **kwargs,
    ...
    self.symbol_table.define(
        func_symbol
    )  # This goes in current scope (should be global)
    ...
    if node.body:
        ...
        for i, (param, llvm_param) in enumerate(zip(node_params, func.args)):
            if param.name == "self" or not param.is_mutable:
                ...
                param_ptr = local_builder.alloca(
                    llvm_param.type, name=f"{param.name}_param"
                )
...
def handle_function_call(self, node, builder: ir.IRBuilder, var_type=None, **kwargs):
    ...
    if isinstance(node, ASTNode.FunctionCall):
        ...
        if hasattr(node, "name"):
            ...
            if hasattr(node.left, "value"):
                ...
                raise ValueError(
                    "Invalid function call format: function name not found"
                )
        else:
        ...
        if hasattr(node, "arguments") and node.arguments is not None:
            ...
            if isinstance(node.right, list):
                ...
                if (
                    hasattr(node.right, "arguments")
                    and node.right.arguments is not None
                ):
    ...
    if len(arguments) != len(expected_arg_types):
        raise ValueError(
            f"Function {func_name} expects {len(expected_arg_types)} arguments, but {len(arguments)} were provided"
        )
    ...
    for i, arg in enumerate(arguments):
        ...
        if hasattr(arg, "node_type") and arg.node_type == NodeType.REFERENCE:
            ...
            if var_name:
                ...
                if (
                    arg_symbol
                    and arg_symbol.is_array
                    and isinstance(expected_type, ir.PointerType)
                ):
                    zero = ir.Constant(ir.IntType(32), 0)
                    llvm_arg = builder.gep(
                        arg_symbol.llvm_value,
                        [zero, zero],
                        inbounds=True,
                        name=f"{arg_symbol.name}_decay",
                    )

        # Case 2: General array literal (like string constants)
        elif isinstance(llvm_arg.type, ir.types.ArrayType) and isinstance(
            expected_type, ir.PointerType
        ):
        ...
        if llvm_arg.type != expected_type:
            try:
                ...
                raise TypeError(
                    f"Cannot cast argument {i+1} from {llvm_arg.type} to {expected_type} for function {func_name}: {e}"
                )
